import { IArrow } from "arrow-drawer";
...
export class ArrowDrawer {
    ...
    public get Width() {
        if (this.Options.start == this.Options.end) return AppConfig.width;
        return Math.abs(this.Options.start - this.Options.end) * AppConfig.width;

    }


    constructor(canvas: HTMLCanvasElement, opt: IArrow) {
...
class ArrowClass {
    padding = { top: 20 }
    Config: ArrowDrawer;
    constructor(config: ArrowDrawer) {
        this.Config = config;

        if (this.Config.Options.start != this.Config.Options.end) this.DrawPointToPoint();
        else  this.DrawSelf();
    }
    ...
    DrawSelf() {       
        ...
        this.self_canvas_arrow()
        this.label_draw()
    }


    DrawPointToPoint() {
        console.log("DRAW", this.Config.Options.Title);

        const ctx = this.Config.ctx;
        ctx.lineWidth = 2;
        if (this.Config.Options.LineStyle == "dashed") ctx.setLineDash([10, 6]); else ctx.setLineDash([]);
        ctx.strokeStyle = this.Config.Options.BorderColor;
        this.canvas_arrow()
        this.label_draw()
    }
    ...
    self_canvas_arrow() {
        ...
        ctx.lineTo(from.x +45, from.y);   
        ctx.lineTo(to.x +45, to.y);
    ...
    label_draw() {
        ...
        if (this.Config.Options.IsMarked) ctx.fillStyle = this.Config.Options.BorderColor;
        ctx.font = "14px segoe ui"
        ctx.fillText(
            this.Config.Options.Title,
            this.Config.Left + this.Config.Width / 2,
            this.padding.top + this.Config.Top);
    ...
    underline(ctx: CanvasRenderingContext2D, text: string, x: number, y: number) {
